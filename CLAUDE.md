...
- **All "feel" tunables live in `src/config.ts`** (timing windows, pitch speed, innings, shake, runner speed, audio volume, the `LIVE` live-play block, and the `MODES` kid/main table: live-sim multipliers + feature flags). Change feel there, not inside scenes.
- **Reusable juice** (shake/burst/floating text/confetti) is in `src/ui/effects.ts`; **character animation** helpers (idle bob, squash-hop with `onDone`, pop-in, `enterFrom` staggered reveals, `pulse` attention loops, `groundShadow`, `runCycle` two-frame texture animation) are in `src/ui/anim.ts`; **sound** (free, code-synthesized incl. the recess `bell()`) is in `src/systems/audio.ts`. Character motion combines pose-texture swaps (`runCycle`) with procedural tweens — e.g. GameScene draws a swinging **bat** prop and a pitcher **wind-up**. The Schoolyard is title + draft in one continuous scene: PLAY rings the bell, doors burst open, kids stream to the brick wall, tap-to-inspect stat cards, picked kids run to their team's pennant, both teams cheer. Feel knobs live in `config.ANIM`.
