...
A **free web baseball game for little kids (ages 4–8)**. You draft 9 of 30 neighborhood characters, then play a short game where you bat the top of each inning and pitch the bottom — one timing button for both. Balls in play become **live plays**: you steer a fielder with the pointer and charge throws on defense, and tap to send runners on offense (EASY/HARD toggle on the title). The real product is **the 30 characters**: the game is a "voting machine" — every player draft pick is tallied, and pick rates reveal which kids should become toys/shows. So two jobs: (1) be genuinely fun, (2) log pick data. Design pillars: minimal reading, icon/voice-forward, short games.
...
- **Balls in play run through the live-play sim** (`systems/liveplay.ts`): `resolveContact` (atbat.ts) turns a contact band into a trajectory, then GameScene's `update()` steps `stepLivePlay(state, inputs, dt)` every frame and positions everything the sim owns (ball, fielders, runners) **directly — never with tweens**. Outs emerge from the race (fly-outs + ball-beats-runner-to-the-bag force-outs only; caught fly returns runners free). CPU policies for the non-human side live inside the sim. `finishLivePlay` → `applyLivePlay` (inning.ts) folds the result back into the half-inning. Field coordinates shared by sim + renderer live in `systems/geometry.ts`. Difficulty (EASY/HARD, persisted) merges `config.LIVE` × `config.DIFFICULTY` via `systems/difficulty.ts`.
...
- **All "feel" tunables live in `src/config.ts`** (timing windows, pitch speed, innings, shake, runner speed, audio volume, the `LIVE` live-play block, and the `DIFFICULTY` easy/hard multiplier table). Change feel there, not inside scenes.
...
| `src/systems/difficulty.ts` | EASY/HARD persistence + `resolveLiveParams` (LIVE × DIFFICULTY). |
