...
@dataclass
class JunebugBuilder(MeshBuilder):
    """Junebug's own two surfaces, which read the measured tables above.

    `hair_cap` is hers outright — a slicked crown against a traced hairline.
    `head_surface` is generic in shape and specific in its inputs: it reads
    `HEAD_CENTER`, `HEAD_RADII`, `FACE_HALF_WIDTH`, the socket and nose pushes
    and the atlas island window, all of which are measurements off HER
    turnaround. It moves into `sculptlib` when a second character has proved
    which of those are really parameters and which are the shape of a child's
    skull; guessing that from one character is how a "shared" library ends up
    sculpting one kid thirty times.
    """
...
def main() -> None:
    ...
    install_face_atlas(FACE_ATLAS, "junebug")
