// ---------------------------------------------------------------------------
// The neighborhood. 30 kids you can draft from.
//
// This file is CONTENT, not logic — add, remove, or rebalance kids freely.
// The 3 signature kids from the pitch use `ability` hooks; everyone else is
// `none` and is defined purely by their stats + look.
//
// Stats are 1-10. Aim for real trade-offs: sluggers can't run, speedsters
// can't hit for power, a few kids are weak-but-adorable, etc. The `visual`
// block drives the flat-mascot art (expression/hair/body/uniform/accessory).
// `voiceGender` (required) matches the kid's spoken voice to who they are —
// it picks the gendered curated-voice sublist + pitch band in systems/voices.ts.
// `draftLine` is the kid's own excited shout when picked (spoken in their
// derived voice — systems/voices.ts); `chatterLines` are optional signature
// field-chatter lines for the personality-forward kids (systems/chatter.ts
// has generic pools for everyone else).
// ---------------------------------------------------------------------------

import type { Character } from './types';

/**
 * The one runtime-created kid id. It lives beside the authored roster ids so
 * character identity still has one source, but is deliberately not in
 * `ROSTER`: custom captains do not become a 31st vote candidate or a model
 * delivery requirement.
 */
export const CUSTOM_PLAYER_ID = 'custom_player';

export const ROSTER: Character[] = [
  // --- The 3 signature kids -------------------------------------------------
  {
    id: 'nostrike',
    name: 'Junebug',
    emoji: '🎯',
    tagline: 'Never misses. Ever.',
    voiceGender: 'girl',
    stats: { contact: 10, power: 3, speed: 6, pitching: 2, fielding: 6 },
    visual: { skin: 1, hair: 'ponytail', hairColor: 1, uniform: 2, accessory: 'headband', expression: 'determined', stance: 'crouch',
      body: { height: 0.92, shoulderW: 42 },
      face: { eyeGap: 16, eyeSize: 1.1, nose: 'dot', mouthW: 0.9 },
      hairSpec: { length: 1.15 },
      outfit: { kind: 'stripeTee', top: 3, bottoms: 'jeans' } },
    ability: 'never_strikes_out',
    draftLine: 'Smart pick. I literally never miss!',
    chatterLines: ['I never miss. Just saying.', 'Watch the ball. That is the whole secret.'],
  },
  {
    id: 'calls_shot',
    name: 'Big Talk Theo',
    emoji: '🗣️',
    tagline: 'Calls his shot. Always wrong.',
    voiceGender: 'boy',
    stats: { contact: 5, power: 6, speed: 5, pitching: 4, fielding: 4 },
    visual: { skin: 3, hair: 'short', hairColor: 3, uniform: 4, accessory: 'cap', expression: 'grin', stance: 'high',
      body: { height: 0.97, shoulderW: 47, headW: 1.06, headH: 1.04 },
      face: { eyeGap: 19, nose: 'wedge', mouthW: 1.2 },
      hairSpec: { volume: 1.06, part: -0.6, wisps: 1 },
      outfit: { kind: 'jacket', top: 0 } },
    ability: 'calls_shot',
    draftLine: 'Best pick of your LIFE, right here!',
    chatterLines: ['This one is going over the fence. Guaranteed.', 'I called it. Whatever happens, I called it.'],
  },
  {
    id: 'wheelchair_ace',
    name: 'Zoom Ramirez',
    emoji: '🌀',
    tagline: "Throws a pitch nobody can hit. Not even him.",
    voiceGender: 'boy',
    stats: { contact: 3, power: 4, speed: 7, pitching: 10, fielding: 7 },
    visual: { skin: 2, hair: 'spiky', hairColor: 0, uniform: 1, accessory: 'wheelchair', expression: 'cool',
      body: { shoulderW: 43, neck: 2 },
      face: { eyeSize: 0.95 },
      hairSpec: { volume: 0.96, wisps: 1 },
      outfit: { kind: 'tee', top: 7, bottoms: 'jeans' } },
    ability: 'unhittable_pitch',
    draftLine: 'Yes! Nobody hits my pitch. Nobody.',
    chatterLines: ['Nobody hits the crazy ball.', 'Even I do not know where it goes.'],
  },

  // --- Sluggers (big power, slow) ------------------------------------------
  {
    id: 'big_lou',
    name: 'Big Lou',
    emoji: '💥',
    tagline: 'Hits it to the moon. Sometimes.',
    voiceGender: 'boy',
    stats: { contact: 5, power: 10, speed: 2, pitching: 3, fielding: 2 },
    visual: { skin: 5, hair: 'buzz', hairColor: 3, uniform: 6, accessory: 'none', expression: 'goofy', bodyType: 'chunky', stance: 'open',
      body: { shoulderW: 56, hipW: 8, belly: 1, neck: -3, headW: 1.02 },
      face: { eyeGap: 20, eyeSize: 0.9, nose: 'wedge', mouthW: 1.15 },
      hairSpec: { volume: 1.05 },
      outfit: { kind: 'stripeTee', top: 2 } },
    ability: 'none',
    draftLine: 'Moon shot time, baby!',
    chatterLines: ['I am hitting this one to the MOON.', 'Somebody hold my sandwich.'],
  },
  {
    id: 'tank',
    name: 'Tank',
    emoji: '🦍',
    tagline: 'Slow, strong, snacking.',
    voiceGender: 'boy',
    stats: { contact: 4, power: 9, speed: 2, pitching: 4, fielding: 3 },
    visual: { skin: 2, hair: 'bald', hairColor: 2, uniform: 3, accessory: 'none', expression: 'determined', bodyType: 'chunky', stance: 'open',
      body: { height: 0.9, shoulderW: 54, hipW: 10, belly: 0.8, neck: -4, headH: 0.96 },
      face: { eyeGap: 15, eyeSize: 0.8, eyeStyle: 'sleepy', nose: 'dot', mouthW: 0.85 },
      outfit: { kind: 'tee', top: 9, bottom: 10 } },
    ability: 'none',
    draftLine: 'Tank smash! After snacks.',
    chatterLines: ['Tank ready.', 'Is it snack time or swing time?'],
  },
  {
    id: 'mimi_mash',
    name: 'Mimi Mash',
    emoji: '🔨',
    tagline: 'Swings for the fence. Only the fence.',
    voiceGender: 'girl',
    stats: { contact: 4, power: 9, speed: 4, pitching: 2, fielding: 3 },
    visual: { skin: 0, hair: 'curly', hairColor: 0, uniform: 0, accessory: 'none', expression: 'grin', stance: 'high',
      body: { height: 0.96, shoulderW: 50, hipW: 4, headW: 1.04 },
      face: { eyeGap: 17, eyeSize: 1.15, mouthW: 1.15 },
      hairSpec: { volume: 1.08, wisps: 2 },
      outfit: { kind: 'hoodie', top: 5, bottoms: 'jeans' } },
    ability: 'none',
    draftLine: 'MASH TIME! Point me at the fence!',
    chatterLines: ['Fence, meet ball. Ball, fence.', 'I only know one swing. The BIG one.'],
  },

  // --- Speedsters (fast, contact-y, low power) -----------------------------
  {
    id: 'turbo',
    name: 'Turbo',
    emoji: '⚡',
    tagline: 'Already on second base.',
    voiceGender: 'boy',
    stats: { contact: 7, power: 3, speed: 10, pitching: 3, fielding: 7 },
    visual: { skin: 1, hair: 'spiky', hairColor: 6, uniform: 4, accessory: 'none', expression: 'cool', bodyType: 'small', stance: 'open',
      body: { height: 0.86, shoulderW: 38, neck: 1 },
      face: { eyeGap: 17, eyeSize: 1.05, nose: 'dot', mouthW: 0.9 },
      hairSpec: { volume: 1.06 },
      outfit: { kind: 'tee', top: 4 } },
    ability: 'none',
    draftLine: 'Fast pick! Get it? Because I am fast!',
    chatterLines: ['I am basically already on second.', 'Blink and you missed me.'],
  },
  {
    id: 'sprout',
    name: 'Sprout',
    emoji: '🌱',
    tagline: 'Tiny. Quick. Sneaky bunts.',
    voiceGender: 'boy',
    stats: { contact: 6, power: 2, speed: 9, pitching: 4, fielding: 6 },
    visual: { skin: 3, hair: 'short', hairColor: 0, uniform: 2, accessory: 'none', expression: 'happy', bodyType: 'small', freckles: true, stance: 'crouch',
      body: { height: 0.82, shoulderW: 36, neck: -1, headW: 1.08, headH: 1.08 },
      face: { eyeSize: 1.25, nose: 'dot', cheeks: 1.3 },
      hairSpec: { volume: 0.92, part: 0.5 },
      outfit: { kind: 'overalls', top: 2 } },
    ability: 'crazy_bunt', // the signature 🤪 CRAZY BUNT card joins his swing stack
    draftLine: 'You picked me?! YES! Tiny but mighty!',
    chatterLines: ['Nobody ever fields my crazy bunt.', 'Watch the ball go ZOOP.'],
  },
  {
    id: 'zippy',
    name: 'Zippy Kwan',
    emoji: '🛼',
    tagline: 'Runs before she hits.',
    voiceGender: 'girl',
    stats: { contact: 7, power: 4, speed: 9, pitching: 3, fielding: 7 },
    visual: { skin: 1, hair: 'pigtails', hairColor: 3, uniform: 5, accessory: 'headband', expression: 'grin', stance: 'crouch',
      body: { height: 0.9, shoulderW: 40, neck: 3 },
      face: { eyeGap: 19, eyeSize: 1.1, mouthW: 1.05 },
      hairSpec: { length: 1.12 },
      outfit: { kind: 'stripeTee', top: 6 } },
    ability: 'none',
    draftLine: 'Zippy is IN! Try to keep up!',
  },

  // --- All-rounders ---------------------------------------------------------
  {
    id: 'ace_kid',
    name: 'Ace',
    emoji: '⭐',
    tagline: 'Good at basically everything.',
    voiceGender: 'boy',
    stats: { contact: 8, power: 7, speed: 7, pitching: 6, fielding: 8 },
    visual: { skin: 2, hair: 'short', hairColor: 1, uniform: 0, accessory: 'cap', expression: 'cool',
      body: { shoulderW: 48, neck: 2 },
      hairSpec: { part: -0.2, wisps: 2 },
      outfit: { kind: 'jacket', top: 4, bottoms: 'jeans' } },
    ability: 'none',
    draftLine: 'Good call. I do it all.',
  },
  {
    id: 'penny',
    name: 'Penny Pockets',
    emoji: '🪙',
    tagline: 'Steady as they come.',
    voiceGender: 'girl',
    stats: { contact: 7, power: 6, speed: 6, pitching: 5, fielding: 7 },
    visual: { skin: 0, hair: 'curly', hairColor: 2, uniform: 2, accessory: 'none', expression: 'happy', freckles: true, stance: 'crouch',
      body: { height: 0.93, shoulderW: 45, hipW: 3, belly: 0.3 },
      face: { eyeSize: 1.05, cheeks: 1.2 },
      hairSpec: { volume: 0.94 },
      outfit: { kind: 'overalls', top: 6 } },
    ability: 'none',
    draftLine: 'Penny is on the team! Lucky you!',
  },
  {
    id: 'dex',
    name: 'Dex',
    emoji: '🧢',
    tagline: 'Quiet. Solid. Reliable.',
    voiceGender: 'boy',
    stats: { contact: 6, power: 6, speed: 6, pitching: 6, fielding: 8 },
    visual: { skin: 4, hair: 'curly', hairColor: 6, uniform: 1, accessory: 'cap', expression: 'cool',
      body: { height: 0.95, shoulderW: 46 },
      face: { eyeStyle: 'button', eyeGap: 16, nose: 'dot', mouthW: 0.85 },
      hairSpec: { volume: 1.05, part: -0.5 },
      outfit: { kind: 'hoodie', top: 9, bottoms: 'jeans' } },
    ability: 'none',
    draftLine: 'Cool. Let us win.',
  },

  // --- Pitchers (arm > bat) -------------------------------------------------
  {
    id: 'lefty',
    name: 'Lefty Lu',
    emoji: '🌪️',
    tagline: 'Curveball from another zip code.',
    voiceGender: 'girl',
    stats: { contact: 4, power: 3, speed: 5, pitching: 9, fielding: 6 },
    visual: { skin: 1, hair: 'ponytail', hairColor: 4, uniform: 4, accessory: 'cap', expression: 'determined',
      body: { shoulderW: 40, neck: 5, headH: 0.97 },
      face: { eyeGap: 15, eyeSize: 0.95, mouthW: 0.9 },
      hairSpec: { volume: 0.94, length: 0.85 },
      outfit: { kind: 'jacket', top: 7 } },
    ability: 'none',
    draftLine: 'My curveball says thank you!',
  },
  {
    id: 'smokey',
    name: 'Smokey',
    emoji: '🔥',
    tagline: 'Pure heat.',
    voiceGender: 'boy',
    stats: { contact: 3, power: 5, speed: 5, pitching: 9, fielding: 5 },
    visual: { skin: 3, hair: 'buzz', hairColor: 3, uniform: 5, accessory: 'none', expression: 'determined',
      body: { height: 0.93, shoulderW: 49, neck: -2 },
      face: { eyeGap: 14, eyeSize: 0.85, nose: 'dot', mouthW: 0.8 },
      hairSpec: { volume: 0.93, part: -0.5 },
      outfit: { kind: 'tee', top: 0 } },
    ability: 'none',
    draftLine: 'You want heat? I brought heat!',
  },
  {
    id: 'bend_it',
    name: 'Bendy Bao',
    emoji: '🎳',
    tagline: 'The ball goes... around?',
    voiceGender: 'boy',
    stats: { contact: 4, power: 4, speed: 6, pitching: 8, fielding: 5 },
    visual: { skin: 2, hair: 'bun', hairColor: 0, uniform: 2, accessory: 'glasses', expression: 'goofy',
      body: { height: 0.94, shoulderW: 42, hipW: -3 },
      face: { eyeGap: 20, eyeSize: 1.1, mouthW: 1.1 },
      hairSpec: { volume: 0.95, length: 1.1 },
      outfit: { kind: 'stripeTee', top: 7 } },
    ability: 'none',
    draftLine: 'Yesss! My pitches go AROUND stuff!',
  },

  // --- Weak-but-cute / underdogs -------------------------------------------
  {
    id: 'noodle',
    name: 'Noodle',
    emoji: '🍜',
    tagline: 'Trying his best!',
    voiceGender: 'boy',
    stats: { contact: 3, power: 2, speed: 4, pitching: 3, fielding: 3 },
    visual: { skin: 0, hair: 'bald', hairColor: 0, uniform: 3, accessory: 'glasses', expression: 'surprised', bodyType: 'small', freckles: true, stance: 'high',
      body: { height: 0.98, shoulderW: 36, hipW: -4, neck: 6, headW: 0.94, headH: 1.06 },
      face: { eyeStyle: 'sleepy', eyeGap: 15, eyeSize: 1.2, mouthW: 0.9, cheeks: 0.6 },
      outfit: { kind: 'stripeTee', top: 11, bottoms: 'jeans' } },
    ability: 'none',
    draftLine: 'Me?! REALLY?! I will try SO hard!',
  },
  {
    id: 'bubbles',
    name: 'Bubbles',
    emoji: '🫧',
    tagline: 'Here for a good time.',
    voiceGender: 'girl',
    stats: { contact: 4, power: 3, speed: 5, pitching: 2, fielding: 4 },
    visual: { skin: 1, hair: 'curly', hairColor: 4, uniform: 5, accessory: 'none', expression: 'happy', freckles: true,
      body: { height: 0.9, shoulderW: 44, hipW: 5, belly: 0.5, headW: 1.05 },
      face: { eyeStyle: 'button', eyeGap: 20, eyeSize: 1.2, nose: 'dot', mouthW: 1.1, cheeks: 1.4 },
      hairSpec: { volume: 0.9, wisps: 1 },
      outfit: { kind: 'dress', top: 6 } },
    ability: 'none',
    draftLine: 'Yay! This is the best recess EVER!',
  },
  {
    id: 'sniffles',
    name: 'Sniffles',
    emoji: '🤧',
    tagline: 'Allergic to the outfield.',
    voiceGender: 'boy',
    stats: { contact: 3, power: 3, speed: 3, pitching: 5, fielding: 2 },
    visual: { skin: 2, hair: 'short', hairColor: 5, uniform: 0, accessory: 'none', expression: 'surprised', bodyType: 'small',
      body: { height: 0.87, shoulderW: 39, neck: -2, headH: 1.03 },
      face: { eyeGap: 16, eyeSize: 1.1, eyeStyle: 'sleepy', nose: 'wedge', mouthW: 0.85, cheeks: 1.2 },
      hairSpec: { volume: 0.9, part: 0.8, wisps: 1 },
      outfit: { kind: 'hoodie', top: 11, bottoms: 'jeans' } },
    ability: 'none',
    draftLine: 'Picked! Achoo! I mean... yay!',
  },

  // --- Personalities / fillers with flavor ---------------------------------
  {
    id: 'the_prof',
    name: 'The Professor',
    emoji: '🤓',
    tagline: 'Calculates the launch angle.',
    voiceGender: 'boy',
    stats: { contact: 6, power: 5, speed: 4, pitching: 7, fielding: 6 },
    visual: { skin: 3, hair: 'short', hairColor: 1, uniform: 1, accessory: 'glasses', expression: 'cool',
      body: { height: 0.95, shoulderW: 41, neck: 4, headW: 1.08, headH: 1.06 },
      face: { eyeGap: 16, eyeSize: 0.9, nose: 'dot', mouthW: 0.8 },
      hairSpec: { volume: 1.04, part: 0.3 },
      outfit: { kind: 'jacket', top: 10, bottoms: 'jeans' } },
    ability: 'none',
    draftLine: 'Statistically, an excellent decision.',
  },
  {
    id: 'diva',
    name: 'Dazzle',
    emoji: '✨',
    tagline: 'Blows a kiss after every hit.',
    voiceGender: 'girl',
    stats: { contact: 7, power: 5, speed: 6, pitching: 3, fielding: 4 },
    visual: { skin: 1, hair: 'long', hairColor: 5, uniform: 3, accessory: 'headband', expression: 'grin', stance: 'high',
      body: { shoulderW: 42, neck: 5 },
      face: { eyeGap: 19, eyeSize: 1.15, nose: 'dot', mouthW: 1.05, cheeks: 1.2 },
      hairSpec: { volume: 1.04, length: 1.2 },
      outfit: { kind: 'dress', top: 5 } },
    ability: 'none',
    draftLine: 'Obviously you picked me. Mwah!',
    chatterLines: ['Watch this. But like, really watch.', 'This next hit is dedicated to me.'],
  },
  {
    id: 'grizz',
    name: 'Grizz',
    emoji: '🐻',
    tagline: 'Grumpy. Powerful. Napping.',
    voiceGender: 'boy',
    stats: { contact: 5, power: 8, speed: 3, pitching: 4, fielding: 4 },
    visual: { skin: 5, hair: 'afro', hairColor: 6, uniform: 4, accessory: 'none', expression: 'determined', bodyType: 'chunky', stance: 'open',
      body: { height: 0.97, shoulderW: 55, hipW: 7, belly: 0.7, neck: -3 },
      face: { eyeGap: 17, eyeSize: 0.85, eyeStyle: 'sleepy', nose: 'wedge', mouthW: 0.9, cheeks: 0.8 },
      hairSpec: { volume: 1.08, wisps: 1 },
      outfit: { kind: 'tee', top: 10 } },
    ability: 'none',
    draftLine: 'Fine. I was napping. Let us play.',
  },
  {
    id: 'flash',
    name: 'Flash Gordon Jr.',
    emoji: '📸',
    tagline: 'Fastest bat in the yard.',
    voiceGender: 'boy',
    stats: { contact: 8, power: 4, speed: 8, pitching: 4, fielding: 7 },
    visual: { skin: 2, hair: 'mohawk', hairColor: 0, uniform: 0, accessory: 'none', expression: 'cool', stance: 'crouch',
      body: { height: 0.96, shoulderW: 39, hipW: -4, neck: 3 },
      face: { eyeGap: 16, eyeSize: 0.9, nose: 'dot', mouthW: 0.9 },
      hairSpec: { volume: 1.08 },
      outfit: { kind: 'stripeTee', top: 0 } },
    ability: 'none',
    draftLine: 'Fastest bat in the yard, reporting in!',
  },
  {
    id: 'cricket',
    name: 'Cricket',
    emoji: '🦗',
    tagline: 'Bounces everywhere.',
    voiceGender: 'girl',
    stats: { contact: 6, power: 3, speed: 8, pitching: 5, fielding: 8 },
    visual: { skin: 0, hair: 'spiky', hairColor: 2, uniform: 2, accessory: 'none', expression: 'goofy', bodyType: 'small', freckles: true, stance: 'crouch',
      body: { height: 0.84, shoulderW: 37, neck: 2, headW: 1.06 },
      face: { eyeGap: 21, eyeSize: 1.25, nose: 'dot', mouthW: 1.1, cheeks: 1.3 },
      hairSpec: { volume: 0.92, part: 0.4 },
      outfit: { kind: 'overalls', top: 3 } },
    ability: 'none',
    draftLine: 'Boing! Picked! Boing boing!',
  },
  {
    id: 'moose',
    name: 'Moose',
    emoji: '🫎',
    tagline: 'Big kid, bigger heart.',
    voiceGender: 'boy',
    stats: { contact: 6, power: 8, speed: 3, pitching: 5, fielding: 5 },
    visual: { skin: 5, hair: 'buzz', hairColor: 1, uniform: 6, accessory: 'cap', expression: 'happy', bodyType: 'chunky', stance: 'open',
      body: { shoulderW: 56, hipW: 6, belly: 0.6, neck: 1, headW: 1.04 },
      face: { eyeGap: 21, eyeSize: 1.05, nose: 'wedge', mouthW: 1.15, cheeks: 1.1 },
      hairSpec: { volume: 1.08, wisps: 1 },
      outfit: { kind: 'hoodie', top: 1 } },
    ability: 'none',
    draftLine: 'Aw yeah! Team hug! Group hug, everybody!',
  },
  {
    id: 'peaches',
    name: 'Peaches',
    emoji: '🍑',
    tagline: 'Sweet swing, sunny smile.',
    voiceGender: 'girl',
    stats: { contact: 8, power: 5, speed: 6, pitching: 4, fielding: 6 },
    visual: { skin: 1, hair: 'bun', hairColor: 2, uniform: 4, accessory: 'none', expression: 'happy', freckles: true,
      body: { height: 0.92, shoulderW: 43, hipW: 4, belly: 0.3 },
      face: { eyeStyle: 'button', eyeSize: 1.1, mouthW: 1.05, cheeks: 1.35 },
      hairSpec: { volume: 1.06, length: 0.9 },
      outfit: { kind: 'dress', top: 1 } },
    ability: 'none',
    draftLine: 'Sweet! Sweetest swing coming right up!',
  },
  {
    id: 'gizmo',
    name: 'Gizmo',
    emoji: '🔧',
    tagline: 'Built his own bat.',
    voiceGender: 'boy',
    stats: { contact: 5, power: 6, speed: 5, pitching: 6, fielding: 6 },
    visual: { skin: 2, hair: 'spiky', hairColor: 2, uniform: 3, accessory: 'glasses', expression: 'determined',
      body: { height: 0.89, shoulderW: 40, neck: 1, headW: 1.05 },
      face: { eyeStyle: 'sleepy', eyeGap: 15, eyeSize: 1.05, mouthW: 0.85 },
      hairSpec: { volume: 1.04, part: -0.5 },
      outfit: { kind: 'overalls', top: 4 } },
    ability: 'none',
    draftLine: 'Great! I brought my custom bat. I built it!',
  },
  {
    id: 'clover',
    name: 'Clover',
    emoji: '🍀',
    tagline: 'Somehow it always works out.',
    voiceGender: 'girl',
    stats: { contact: 6, power: 6, speed: 7, pitching: 4, fielding: 5 },
    visual: { skin: 0, hair: 'pigtails', hairColor: 4, uniform: 2, accessory: 'none', expression: 'happy', freckles: true,
      body: { height: 0.91, shoulderW: 41 },
      face: { eyeSize: 1.05, nose: 'dot', cheeks: 1.2 },
      hairSpec: { volume: 1.05, length: 0.88 },
      outfit: { kind: 'dress', top: 3 } },
    ability: 'none',
    draftLine: 'Lucky you! No really, I AM the luck!',
  },
  {
    id: 'rocket',
    name: 'Rocket Rosa',
    emoji: '🚀',
    tagline: 'Blasts off down the line.',
    voiceGender: 'girl',
    stats: { contact: 7, power: 5, speed: 9, pitching: 3, fielding: 6 },
    visual: { skin: 3, hair: 'ponytail', hairColor: 0, uniform: 0, accessory: 'none', expression: 'cool',
      body: { height: 0.98, shoulderW: 44, hipW: -2, neck: 3 },
      face: { eyeGap: 17, mouthW: 0.95 },
      hairSpec: { length: 1.05, part: 0.6 },
      outfit: { kind: 'tee', top: 8 } },
    ability: 'none',
    draftLine: 'Rosa, ready for liftoff!',
  },
  {
    id: 'chip',
    name: 'Chip',
    emoji: '🐿️',
    tagline: 'Little bat, quick feet.',
    voiceGender: 'boy',
    stats: { contact: 6, power: 3, speed: 7, pitching: 5, fielding: 7 },
    visual: { skin: 2, hair: 'short', hairColor: 2, uniform: 5, accessory: 'cap', expression: 'happy', bodyType: 'small', freckles: true,
      body: { height: 0.85, shoulderW: 38, neck: -1, headW: 1.04 },
      face: { eyeStyle: 'button', eyeGap: 19, eyeSize: 1.15, nose: 'dot', mouthW: 1.05, cheeks: 1.2 },
      hairSpec: { volume: 0.95, part: -0.9, wisps: 3 },
      outfit: { kind: 'hoodie', top: 3 } },
    ability: 'none',
    draftLine: 'Chip is on the team! Quick feet, coming through!',
  },
  {
    id: 'boomer',
    name: 'Boomer',
    emoji: '📣',
    tagline: 'Loud. Very loud.',
    voiceGender: 'boy',
    stats: { contact: 5, power: 7, speed: 5, pitching: 5, fielding: 4 },
    visual: { skin: 4, hair: 'mohawk', hairColor: 6, uniform: 1, accessory: 'none', expression: 'grin', stance: 'high',
      body: { height: 0.95, shoulderW: 51, hipW: 2, belly: 0.4, neck: -2, headW: 1.03 },
      face: { nose: 'wedge', mouthW: 1.25 },
      hairSpec: { volume: 0.92, part: 0.4 },
      outfit: { kind: 'stripeTee', top: 1 } },
    ability: 'none',
    draftLine: 'BOOMER! IS! ON! THE! TEAM!',
    chatterLines: ['HEY BATTER BATTER BATTER!', 'THIS IS ME BEING QUIET!'],
  },
];

/** Fast lookup by id. */
const BY_ID = new Map(ROSTER.map((c) => [c.id, c]));

export function getCharacter(id: string): Character {
  const c = BY_ID.get(id);
  if (!c) throw new Error(`Unknown character id: ${id}`);
  return c;
}
