...
| 3.12 | Legs are two legs — daylight visible between the thighs and between the calves in bind pose, idle and run; overlapping leg volumes read as one mass | BOARD + EYES. ⚠️ The AUTO half reads the BIND pose only: a signature idle that rolls the legs inward fails this while every measured metric stays green (`render.leg-stance`) |
...
| 3.14 | The mouth's emotion is readable at draft-card distance, not only in close-up — lips, teeth and tongue where a cell opens the mouth, never a stroke that collapses to a line | EYES — the four `<name>-runtime-face-*.png` stills `capture:character-evidence` writes |
