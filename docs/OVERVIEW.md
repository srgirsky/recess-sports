...
- **Title** — a choreographed intro: bunting drops in, the logo slams down (with a thump of screen shake), the showcase kids RUN in from the sides and settle into bobbing, and a pulsing PLAY button (unlocks audio). Ambient life keeps it moving: drifting clouds, a breathing sun glow, and a random kid hopping every few seconds. Mute toggle, dev pick-rate overlay (press D).
- **Draft** — 30 cards; player and AI alternate picks to 9 each; every player pick is logged. Staged as an event: the grid *deals in* card by card, your pick is announced by voice and FLIES to the dugout bench (landing with a hop + burst), the CPU visibly *scans* candidates with a red spotlight before grabbing one, the turn pill pulses when it's your go, and the finished team celebrates with confetti and a bench-wide hop wave before fading into the game.
...
- **`ui/anim.ts`** — procedural character animation helpers (idle "breathing" bob, celebratory squash-hop, pop-in, `enterFrom` staggered reveals, `pulse` attention loops, `runIn` off-screen dashes). Motion is tweens on the single-texture sprites — no frame art. In-game: a swinging **bat** prop, a pitcher **wind-up**, and baserunners rendered as the **actual kids** (containers) that bounce as they run and hop when they score. Scene transitions use quick camera fades. Timing lives in `config.ANIM`.
