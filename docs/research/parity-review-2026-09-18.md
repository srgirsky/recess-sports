...
| --- | --- | --- |
| Character identity | Clear mismatch between some illustrated portraits and the corresponding 3D kid: Junebug is green in the draft card and red in the scene. The custom captain is especially inconsistent. | Recognize the same child in the card, draft, gameplay and results by face, hair, silhouette and signature clothing. |
...
| First-session usability | Startup was blank until assets finished; the draft lacked Back and autofill; gameplay offered no basic control explanation. This branch improves all three. | A child can reach play and perform each verb without an adult explaining hidden controls. Text hints alone do not prove this for nonreaders. |
...
[the existing ledger](../v2/art/reviews.json), ART-007 through ART-009. Existing
...
2. Reconcile portraits with approved 3D identities, including the custom captain.
...
## Verification for the first correction pass

- `npm test -- --maxWorkers=2`: 116 files passed, 2,311 tests passed,
...
- `npm run build`: passed; the classic output remains `classic-Di4uhfdr.js`.
  The v2 entry loads about 1,046 kB; the unchanged bundle gate passes.
...
- The final responsive audit passed 93 scenarios (5,205 boxes), including
...
The remaining work stays in this task's review/fix loop. Start the next pass
with portrait/model identity: choose a representation derived from the same
runtime character rather than recolouring an unrelated illustration. Preserve
custom-player support, model-loader ownership, device budgets and v1 art. Then
work through the sculpt/action findings, audio, and complete-game playability.
Human child-playtest observations and independent approval must never be
invented. Do not call the game ready merely because this first PR is green.
