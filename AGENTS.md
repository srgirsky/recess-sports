...
| `scripts/v2/silhouette.lint.test.js` / `performance-freshness.lint.test.js` / `tonemapping.lint.test.js` / `sculptspec.lint.test.js` / `evidence-freshness.lint.test.js` | a hole you can see through, a baked animation take stale against the code that bakes it, a colour default reverted silently, one kid's measurement defaulted into the shared sculpt library, a character scored from stills of an older model |
