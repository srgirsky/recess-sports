# Spike harvest — technique inventory for the Phase B port

Source: spike-bb26 (`spike/bb26-one-shot`), exit scorecard `critic/verdict-final.md`.
The exit call harvested **all six dimensions** — the spike beats the v2 baseline
everywhere (silhouette 7v2, face 7v2, motion 8v3, venue 8v2, HUD 9v1, vibe 8v2).
Entries below are ordered by expected impact on the character-quality gap:
characters and motion first, then camera/vibe, then HUD, then venue and
texture recipes. Each entry names the spike mechanism (file, function, numbers)
so the port is a restatement, not a rediscovery.
...
- **PORT TARGET:** `scripts/v2/blender/sculpt-*.py` helpers — bake the ratio
...
- **PORT TARGET:** `scripts/v2/blender/sculpt-*.py` helpers — the boundary
...
- **PORT TARGET:** `scripts/v2/blender/sculpt-*.py` — author the mitt as one
...
- **PORT TARGET:** `scripts/v2/blender/sculpt-*.py` — carry the exclusion rule
...
- **Procedural hero faces/hair as an end state** — verdict-final's ceiling
  section is unambiguous: expression acting, signature hair identity, and
  painterly texture warmth need authored assets. Entries 1–4 feed the
