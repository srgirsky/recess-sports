...
export function rollFace(rng: KidRng): FaceRecipe {
  return {
    ...
    eyeGap: rng.range(0.24, 0.29),
    // Sclera ~1/4 head width: head is 1.68 wide, so whites are ~0.36-0.42 across.
    eyeSize: rng.range(0.18, 0.21),
    noseSize: rng.range(0.095, 0.125),
...
export function buildFace(mats: MatCache, skinKey: string, hairKey: string, f: FaceRecipe): THREE.Group {
  ...
  const white = mats.glow('cloudLit');
  ...
  for (const s of [-1, 1]) {
    const x = s * f.eyeGap;
    g.add(blob(white, f.eyeSize, [0.95, 1.25, 0.5], [x, 0.08, 0.66]));
    // Pupil: big and dark, sitting on the sclera's front face.
    g.add(blob(ink, 0.082, [1, 1.2, 0.45], [x - s * 0.02, 0.05, 0.66 + f.eyeSize * 0.48], 12));
    g.add(blob(mats.glow('cloudLit', 1, 0.8), 0.03, [1, 1, 0.7], [x + s * 0.015, 0.11, 0.69 + f.eyeSize * 0.48], 8));
    // Brow: thick sculpted dash in a darkened hair tone.
    const b = blob(brow, 0.06, [2.6, 0.8, 0.6], [x + s * 0.01, 0.34, 0.64], 10);
    b.rotation.z = s * -f.browTilt;
    g.add(b);
  }

  // Chunky nose, slightly darker than the skin so it reads as a form.
  g.add(blob(skinDark, f.noseSize, [1.05, 0.85, 0.95], [0, -0.1, 0.74], 14));
  ...
  // Mouth — sized to read at the batter-in-pitching-view distance.
  if (f.mouth === 'open') {
    g.add(blob(ink, 0.2, [1.25, 0.85, 0.35], [0, -0.38, 0.58])); // dark interior
    g.add(blob(mats.glow('cloudLit', 1, 0.45), 0.12, [1.5, 0.4, 0.42], [0, -0.27, 0.62], 12)); // teeth strip
    g.add(blob(mats.get('hudRed', 0.85), 0.1, [1.15, 0.55, 0.5], [0, -0.46, 0.6], 10)); // tongue
  } else if (f.mouth === 'smile') {
    const arc = new THREE.Mesh(new THREE.TorusGeometry(0.18, 0.04, 8, 20, 2.1), ink);
    arc.position.set(0, -0.28, 0.64);
    arc.rotation.z = Math.PI + (Math.PI - 2.1) / 2;
    g.add(arc);
    // Lower-lip hint under the smile so it reads as a mouth, not a scratch.
    g.add(blob(skinDark, 0.07, [1.6, 0.5, 0.5], [0, -0.44, 0.6], 10));
  } else {
    g.add(blob(ink, 0.048, [3.2, 0.7, 0.5], [0, -0.34, 0.64], 10));
  }
